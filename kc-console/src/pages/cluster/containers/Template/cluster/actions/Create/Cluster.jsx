...
import {
  ...
  ciliumKubeProxyReplacementOptions,
...
export default class Cluster extends BaseForm {
  ...
  updateFormVersions(versions) {
    ...
    this.updateFormValue('ciliumVersion', ciliumVersion);
  }
  ...
  getCurrentVersionsByK8s = (kubernetesVersion) => {
    ...
    const ciliumVersions = filter(cni, ['name', 'cilium']);
    ...
    const ciliumVersionArr = find(ciliumVersions, ['default', true]);
    ...
    return {
      ...
      ciliumVersions,
      ciliumVersion: ciliumVersionArr?.version,
    };
  };
  ...
  onCniTypeChange = (type) => {
    this.updateContext({ cniType: type });
    const { calicoVersions = [], ciliumVersions = [] } = this.props.context;
    if (type === 'calico') {
      const version =
        find(calicoVersions, ['default', true])?.version ||
        calicoVersions[0]?.version;
      this.updateFormValue('calicoVersion', version);
    }
    if (type === 'cilium') {
      const version =
        find(ciliumVersions, ['default', true])?.version ||
        ciliumVersions[0]?.version;
      this.updateFormValue('ciliumVersion', version);
      if (!this.props.context.ciliumIPv4Pool?.length) {
        this.updateFormValue('ciliumIPv4Pool', clusterParams.ciliumIPv4Pool);
      }
      if (!this.props.context.ciliumIPv4MaskSize) {
        this.updateFormValue(
          'ciliumIPv4MaskSize',
          clusterParams.ciliumIPv4MaskSize
        );
      }
      if (!this.props.context.kubeProxyReplacement) {
        this.updateFormValue(
          'kubeProxyReplacement',
          clusterParams.kubeProxyReplacement
        );
      }
      if (!this.props.context.ciliumOperatorReplicas) {
        this.updateFormValue(
          'ciliumOperatorReplicas',
          clusterParams.ciliumOperatorReplicas
        );
      }
    }
  };
  ...
  get formItems() {
    ...
    return [
      ...
      [
        ...
        {
          name: 'ciliumVersion',
          label: t('Cilium Version'),
          type: 'select',
          required: true,
          options: this.getMetaVersion(this.props.context.ciliumVersions),
          hidden: !this.isCilium,
        },
        {
          name: 'calicoMode',
          label: t('Calico Mode'),
        ...
        },
        {
          ...
          rules: [
            {
              validator: (rule, value = []) => {
                const isInValide = value.some(
                  (it) => it.value && !cidrRegex({ exact: true }).test(it.value)
                );
                if (isInValide) {
                  return Promise.reject(
                    t('The enter does not meet ipv4 cidr specification!')
                  );
                }
                return Promise.resolve(true);
              },
          ...
          tip: t('Configure the IPv4 address pools used by Cilium IPAM.'),
        },
        {
          name: 'ciliumIPv4MaskSize',
          label: t('IPv4 Mask Size'),
          type: 'input-number',
          min: 1,
          max: 32,
          hidden: !this.isCilium,
          required: true,
          tip: t(
            'Subnet mask size for allocating pod CIDRs from the address pools.'
          ),
        },
        {
          name: 'kubeProxyReplacement',
          label: t('Kube Proxy Replacement'),
          type: 'select',
          options: ciliumKubeProxyReplacementOptions,
          hidden: !this.isCilium,
          required: true,
        },
        {
          name: 'ciliumOperatorReplicas',
          label: t('Operator Replicas'),
          type: 'input-number',
          min: 1,
          hidden: !this.isCilium,
          required: true,
