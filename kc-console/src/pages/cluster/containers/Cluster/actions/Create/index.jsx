...
export default class Create extends StepAction {
  ...
  onSubmit = (values) => {
    ...
    const cniVersion = cniType === 'calico' ? calicoVersion : ciliumVersion;
    const ciliumPodCIDRs = arrayInputValue(ciliumIPv4Pool);
    const cni = {
      type: cniType,
      version: cniVersion,
    };
    ...
    if (cniType === 'cilium') {
      cni.cilium = {
        kubeProxyReplacement,
        operator: {
          replicas: ciliumOperatorReplicas,
        },
        ipam: {
          operator: {
            clusterPoolIPv4PodCIDRList: ciliumPodCIDRs,
            clusterPoolIPv4MaskSize: ciliumIPv4MaskSize,
          },
        },
      };
    }
