...
const ClusterTemplateMapper = (item) => {
  ...
  return {
    ...
    calicoVersion: get(config, 'cni.type') === 'calico'
      ? get(config, 'cni.version')
      : undefined,
    ciliumVersion: get(config, 'cni.type') === 'cilium'
      ? get(config, 'cni.version')
      : undefined,
    ...
    kubeProxyReplacement: get(config, 'cni.cilium.kubeProxyReplacement'),
    ciliumOperatorReplicas: get(config, 'cni.cilium.operator.replicas'),
    ciliumIPv4Pool: (get(
      config,
      'cni.cilium.ipam.operator.clusterPoolIPv4PodCIDRList',
      []
    ) || []).map((val, index) => ({ value: val, index })),
    ciliumIPv4MaskSize: get(
      config,
      'cni.cilium.ipam.operator.clusterPoolIPv4MaskSize'
    ),
